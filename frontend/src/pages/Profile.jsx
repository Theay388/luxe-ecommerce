      ...
      <div className="container" style={{ padding: 'var(--space-8) var(--space-4)' }}>
        <div className="profile-layout">
          <aside className="profile-sidebar">
            ...
            <p style={{ fontSize: '0.75rem', color: 'var(--color-muted)', marginBottom: 'var(--space-6)' }}>Member</p>
            {tabs.map((t, i) => (
            ...
            <button className="btn-ghost" style={{ marginTop: 'var(--space-6)', fontSize: '0.75rem', paddingLeft: 'var(--space-4)' }} onClick={() => { logout(); navigate('/') }}>SIGN OUT</button>
          </aside>
          
          {/* Mobile Tabs */}
          <div className="profile-tabs">
            {tabs.map((t, i) => (
              <button key={t} onClick={() => setTab(i)} className={`profile-tab ${tab === i ? 'active' : ''}`}>{t}</button>
            ))}
          </div>
          ...
          <div className="profile-content">
            {tab === 0 && (
              <div>
                <h2 style={{ fontFamily: 'var(--font-serif)', marginBottom: 'var(--space-6)' }}>My Information</h2>
                <form onSubmit={saveProfile}>
                  <p className="text-uppercase text-small text-muted" style={{ marginBottom: 'var(--space-4)' }}>Personal Details</p>
                ...
                <p className="text-uppercase text-small text-muted" style={{ marginBottom: 'var(--space-4)' }}>Change Password</p>
            ...
            {tab === 1 && (
              <div>
                <h2 style={{ fontFamily: 'var(--font-serif)', marginBottom: 'var(--space-6)' }}>My Orders</h2>
                {orders.length === 0 ? <p style={{ color: 'var(--color-muted)' }}>No orders yet.</p> : orders.map(o => (
                  <div key={o._id} onClick={() => navigate(`/orders/${o._id}`)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 'var(--space-4)', background: 'var(--color-bg-low)', marginBottom: 'var(--space-3)', cursor: 'pointer', flexWrap: 'wrap', gap: 'var(--space-3)' }}>
                    <div>
            ...
            {tab === 2 && (
              <div>
                <h2 style={{ fontFamily: 'var(--font-serif)', marginBottom: 'var(--space-6)' }}>Settings</h2>
